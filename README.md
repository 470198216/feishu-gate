...
飞书发「给拓扑加导出按钮」会收到【审核卡】，**不会立刻改文件**。

- 回复 `通过`：用 Cursor SDK 改 `WRITE_CWD`（默认拓扑仓库），不提交 git。
- 回复 `驳回` 或 `驳回：原因`：取消。
- 多单待审时：`通过 job-xxxx`
- `删除全部数据` 这类 destroy：通过也只归档，不会真执行。

`CURSOR_API_KEY` 可写在 `feishu-gate/.env`，不写则自动用 `cursor-lan/.env` 里那把。启动窗口会打印 `key=已配` 或 `缺失`。

改代码可能要一两分钟，飞书会先回「开始改」，写完再推结果。同时只跑一单。
